...
/**************************version RN2.5*********************************/
	...
	$bbcode_tpl['twitter_last'] = '\'' . $bbcode_tpl['twitter_last'] . '\'';
	$bbcode_tpl['twitter_last'] = str_replace('{USERNAME}', "' . trim(str_replace('\\\"', '\"', '\\1')) . '", $bbcode_tpl['twitter_last']);
	$bbcode_tpl['twitter_last'] = str_replace('{USERID}', "' . trim(str_replace('\\\"', '\"', preg_replace('/[^a-z0-9_\- ]/i', '', str_replace(array('&quot;', '&amp;', '?', '.', '\''), '', '\\1')))) . '", $bbcode_tpl['twitter_last']);
	$bbcode_tpl['search'] = '\'' . $bbcode_tpl['search'] . '\'';
	$bbcode_tpl['search'] = str_replace('{STRING}', "' . str_replace('\\\"', '\"', '\\1') . '", $bbcode_tpl['search']);
	$bbcode_tpl['search'] = str_replace('{KEYWORD}', "' . urlencode(trim(str_replace('\\\"', '\"', '\\1'))) . '", $bbcode_tpl['search']);
	$bbcode_tpl['left'] = str_replace('{URL}', '\\1', $bbcode_tpl['left']);
	$bbcode_tpl['right'] = str_replace('{URL}', '\\1', $bbcode_tpl['right']);
	$bbcode_tpl['duck'] = '\'' . $bbcode_tpl['duck'] . '\'';
	$bbcode_tpl['duck'] = str_replace('{STRING}', "' . trim(str_replace('\\\"', '\"', '\\1')) . '", $bbcode_tpl['duck']);
	$bbcode_tpl['duck'] = str_replace('{QUERY}', "' . urlencode(trim(str_replace('\\\"', '\"', '\\1'))) . '", $bbcode_tpl['duck']);
	$bbcode_tpl['wiki'] = '\'' . $bbcode_tpl['wiki'] . '\'';
	$bbcode_tpl['wiki'] = str_replace('{WIKI}', '\\1', $bbcode_tpl['wiki']);
	$bbcode_tpl['wiki'] = str_replace('{STRING}', "' . trim(str_replace('\\\"', '\"', '\\2')) . '", $bbcode_tpl['wiki']);
	$bbcode_tpl['wiki'] = str_replace('{QUERY}', "' . urlencode(str_replace('\\\"', '\"', str_replace(' ', '_', '\\2'))) . '", $bbcode_tpl['wiki']);
	$bbcode_tpl['wiki_default'] = '\'' . $bbcode_tpl['wiki_default'] . '\'';
	$bbcode_tpl['wiki_default'] = str_replace('{STRING}', "' . str_replace('\\\"', '\"', '\\1') . '", $bbcode_tpl['wiki_default']);
	$bbcode_tpl['wiki_default'] = str_replace('{QUERY}', "' . urlencode(str_replace('\\\"', '\"', str_replace(' ', '_', '\\1'))) . '", $bbcode_tpl['wiki_default']);
	...
	$bbcode_tpl['pre_open'] = str_replace('{U_SXBB_JSLIB}', $lang['ModSelectExpand_JS'], $bbcode_tpl['pre_open']);
...
//+MOD: Select Expand BBcodes MOD
   ...
   $expand_ary2 = array("\r<!--\r", "\r//-->\r", $lang['Select'], $lang['Expand'], $lang['Contract'], $lang['ModSelectExpand_JS']);
   $expand_ary3 = array('<!--', '//-->');
...
/**************************version RN2.5*********************************/
	...
	$patterns[] = "#\[twitter:$uid\](.*?)\[/twitter:$uid\]#ise";
	$replacements[] = $bbcode_tpl['twitter_last'];

	$patterns[] = "#\[tag:$uid\](.*?)\[/tag:$uid\]#ise";
	...
	// duckduckgo
	$patterns[] = "#\[duck:$uid\](.*?)\[/duck:$uid\]#ise";
	$replacements[] = $bbcode_tpl['duck'];

	// wikipedia
	$patterns[] = "#\[wiki:$uid\](.*?)\[/wiki:$uid\]#ise";
	$replacements[] = $bbcode_tpl['wiki_default'];

	$patterns[] = "#\[wiki=(en|de|fr|nl|it|pl|es|ru|ja|pt|sv|zh|uk|vi|ca|no|fi|cs|hu|ko|id|tr|ro|fa|ar|da|eo|sr|lt|sk|sl|ms|he|bg|kk|eu|vo|war|hr|hi|et|az|gl|nn|simple|th|la|el|new|tl|sh|ka|mk|ht|pms|te|ta|ceb|br|sq|lv|be|jv|mg|cy|lb|mr|is|bs|ya|an|bpy|hy|lmo|fy|sw|bn|ml|io|gu|af|pnb|ne|nds|scn|ku|ur|su|qu|diq|ba|ast|tt|ga|nap|ia):$uid\](.*?)\[/wiki:$uid\]#ise";
...
/**************************version RN2.5*********************************/
	// strip empty bbcodes
	$text = preg_replace("#\[(wiki|duck|twitter|tag|class|align|font|stream|archive|s|spoil|sub|sup|youtube|pre)(?:=((\\\\&quot;)?[_a-zA-Z0-9- ]*(\\\\&quot;)?))?\]\[/\\1\]#si", "", $text);
	
    // twitter latest
	$text = preg_replace("#\[twitter\](.*?)\[/twitter\]#si", "[twitter:$uid]\\1[/twitter:$uid]", $text);
	
	// search tags
	$text = preg_replace("#\[tag\](.*?)\[/tag\]#si", "[tag:$uid]\\1[/tag:$uid]", $text);
	
	// Floating Images
	$text = preg_replace("#\[imgleft\]((http|ftp|https|ftps)://)([^ \?&=\#\"\n\r\t<]*?(\.(jpg|jpeg|gif|png)))\[/imgleft\]#sie", "'[imgleft:$uid]\\1' . str_replace(' ', '%20', '\\3') . '[/imgleft:$uid]'", $text);
	$text = preg_replace("#\[imgright\]((http|ftp|https|ftps)://)([^ \?&=\#\"\n\r\t<]*?(\.(jpg|jpeg|gif|png)))\[/imgright\]#sie", "'[imgright:$uid]\\1' . str_replace(' ', '%20', '\\3') . '[/imgright:$uid]'", $text);
...
function make_clickable($text)
{
	...
	~ix', 
	'\\1<div class="forumvideo video"><iframe src="http://www.youtube.com/embed/\\2" frameborder="0" allowfullscreen></iframe>
